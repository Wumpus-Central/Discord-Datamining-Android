...
public final class SystemLogUtils {
    ...
    private static final Regex regexExtractTombstone = new Regex("(?:^(?:[^\\s]+\\s+){4}F\\s+DEBUG\\s+:\\s(.+))|(^.+(\\[" + Regex.f21106l.c(LIBDISCORD_VERSION_CATEGORY) + "\\].+))");
    ...
    private final String hashString(String str) {
        ...
        byte[] bytes = str.getBytes(d.f24685b);
