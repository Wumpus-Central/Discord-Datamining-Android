...
public final class SystemLogUtils {
    ...
    private static final Regex regexExtractTombstone = new Regex("(?:^(?:[^\\s]+\\s+){4}F\\s+DEBUG\\s+:\\s(.+))|(^.+(\\[" + Regex.f20754l.c(LIBDISCORD_VERSION_CATEGORY) + "\\].+))");
    ...
    private final String hashString(String str) {
        ...
        byte[] bytes = str.getBytes(d.f20634b);
