...
public final class ContactSyncBlobEntry {
    ...
    public static final void write$Self(ContactSyncBlobEntry self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.t(serialDesc, 0, self.phone);
        output.t(serialDesc, 1, self.unencryptedName);
        output.s(serialDesc, 2, self.hasImageData);
        output.t(serialDesc, 3, self.deviceContactId);
        output.t(serialDesc, 4, self.givenName);
        output.t(serialDesc, 5, self.familyName);
    }
