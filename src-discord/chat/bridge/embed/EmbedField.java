...
public final class EmbedField {
    ...
    public static final void write$Self(EmbedField self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        boolean z13 = false;
        if (!output.A(serialDesc, 0) && self.rawName == null) {
            z10 = false;
        } else {
            z10 = true;
        }
        if (z10) {
            output.u(serialDesc, 0, a2.f32924a, self.rawName);
        }
        if (!output.A(serialDesc, 1) && self.rawValue == null) {
            z11 = false;
        } else {
            z11 = true;
        }
        if (z11) {
            output.u(serialDesc, 1, a2.f32924a, self.rawValue);
        }
        if (!output.A(serialDesc, 2) && self.name == null) {
            z12 = false;
        } else {
            z12 = true;
        }
        if (z12) {
            output.u(serialDesc, 2, StructurableTextSerializer.INSTANCE, self.name);
        }
        if (output.A(serialDesc, 3) || self.value != null) {
            z13 = true;
        }
        if (z13) {
            output.u(serialDesc, 3, StructurableTextSerializer.INSTANCE, self.value);
        }
    }
