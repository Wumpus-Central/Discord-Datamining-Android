...
public final class MessageReaction implements ReactionView.Reaction {
    ...
    public static final void write$Self(MessageReaction self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        boolean z13 = false;
        output.w(serialDesc, 0, self.getCount());
        output.x(serialDesc, 1, self.f6929me);
        if (!output.z(serialDesc, 2) && !self.isMeBurst()) {
            z10 = false;
        } else {
            z10 = true;
        }
        if (z10) {
            output.x(serialDesc, 2, self.isMeBurst());
        }
        output.i(serialDesc, 3, MessageReactionEmoji$$serializer.INSTANCE, self.getEmoji());
        if (!output.z(serialDesc, 4) && self.getBurstCount() == 0) {
            z11 = false;
        } else {
            z11 = true;
        }
        if (z11) {
            output.w(serialDesc, 4, self.getBurstCount());
        }
        if (!output.z(serialDesc, 5) && self.getThemedBurstColors() == null) {
            z12 = false;
        } else {
            z12 = true;
        }
        if (z12) {
            output.E(serialDesc, 5, ThemedBurstReactionColorPalette$$serializer.INSTANCE, self.getThemedBurstColors());
        }
        if (output.z(serialDesc, 6) || self.isMe() != self.f6929me) {
            z13 = true;
        }
        if (z13) {
            output.x(serialDesc, 6, self.isMe());
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        return getCount() == messageReaction.getCount() && this.f6929me == messageReaction.f6929me && isMeBurst() == messageReaction.isMeBurst() && q.c(getEmoji(), messageReaction.getEmoji()) && getBurstCount() == messageReaction.getBurstCount() && q.c(getThemedBurstColors(), messageReaction.getThemedBurstColors());
    }
