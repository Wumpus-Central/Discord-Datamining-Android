...
public final class PostPreviewEmbed {
    ...
    public static final void write$Self(PostPreviewEmbed self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        boolean z13 = false;
        output.q(serialDesc, 0, channelId$$serializer, ChannelId.m546boximpl(self.parentChannelId));
        output.q(serialDesc, 1, channelId$$serializer, ChannelId.m546boximpl(self.threadId));
        output.q(serialDesc, 2, MessageId$$serializer.INSTANCE, MessageId.m572boximpl(self.messageId));
        output.q(serialDesc, 3, GuildId$$serializer.INSTANCE, GuildId.m559boximpl(self.guildId));
        output.z(serialDesc, 4, self.title);
        output.z(serialDesc, 5, self.ctaText);
        output.q(serialDesc, 6, StructurableTextSerializer.INSTANCE, self.footer);
        ...
        if (z10) {
            output.u(serialDesc, 7, a2.f32924a, self.subtitle);
        }
        ...
        if (z11) {
            output.u(serialDesc, 8, a2.f32924a, self.coverImage);
        }
        ...
        if (z12) {
            output.u(serialDesc, 9, a2.f32924a, self.blurredCoverImage);
        }
        if (output.A(serialDesc, 10) || self.coverImageOverlayText != null) {
            z13 = true;
        }
        if (z13) {
            output.u(serialDesc, 10, a2.f32924a, self.coverImageOverlayText);
        }
    }
