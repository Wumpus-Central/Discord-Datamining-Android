...
public final class ActionRowComponent extends Component {
    ...
    public static final void write$Self(ActionRowComponent self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.h(serialDesc, 1, new wi.f(m0.f29615a), self.getIndices());
        if (output.w(serialDesc, 2) || self.errorText != null) {
            z10 = true;
        }
        if (z10) {
            output.z(serialDesc, 2, a2.f29540a, self.errorText);
        }
        output.h(serialDesc, 3, new wi.f(ComponentSerializer.INSTANCE), self.components);
    }
