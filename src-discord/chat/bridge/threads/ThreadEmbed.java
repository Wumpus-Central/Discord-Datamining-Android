...
public final class ThreadEmbed {
    ...
    public static final void write$Self(ThreadEmbed self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.z(serialDesc, 1, self.messageCountLabel);
        if (!output.A(serialDesc, 2) && self.messagePreviewString == null) {
            z10 = false;
        } else {
            z10 = true;
        }
        if (z10) {
            output.u(serialDesc, 2, a2.f32924a, self.messagePreviewString);
        }
        if (!output.A(serialDesc, 3) && self.archived == null) {
            z11 = false;
        } else {
            z11 = true;
        }
        if (z11) {
            output.u(serialDesc, 3, h.f32966a, self.archived);
        }
        if (!output.A(serialDesc, 4) && self.archivedIconUrl == null) {
            z12 = false;
        } else {
            z12 = true;
        }
        if (z12) {
            output.u(serialDesc, 4, a2.f32924a, self.archivedIconUrl);
        }
        if (output.A(serialDesc, 5) || self.referencedMessage != null) {
            z13 = true;
        }
        if (z13) {
            output.u(serialDesc, 5, ThreadEmbedMessage$$serializer.INSTANCE, self.referencedMessage);
        }
    }
