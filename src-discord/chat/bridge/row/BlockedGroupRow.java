...
public final class BlockedGroupRow extends Row {
    ...
    public static final void write$Self(BlockedGroupRow self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.x(serialDesc, 2, self.color);
        output.x(serialDesc, 3, self.borderColor);
        output.x(serialDesc, 4, self.backgroundColor);
        output.z(serialDesc, 5, self.text);
        output.y(serialDesc, 6, self.revealed);
        output.q(serialDesc, 7, BlockedGroupButton$$serializer.INSTANCE, self.button);
        if (output.A(serialDesc, 8) || self.content != null) {
            z10 = true;
        }
        if (z10) {
            output.u(serialDesc, 8, new yj.f(BlockedGroupContent$$serializer.INSTANCE), self.content);
        }
    }
