...
public final class BlockedGroupRow extends Row {
    ...
    public static final void write$Self(BlockedGroupRow self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.w(serialDesc, 2, self.color);
        output.w(serialDesc, 3, self.borderColor);
        output.w(serialDesc, 4, self.backgroundColor);
        output.y(serialDesc, 5, self.text);
        output.x(serialDesc, 6, self.revealed);
        output.z(serialDesc, 7, BlockedGroupButton$$serializer.INSTANCE, self.button);
        if (output.A(serialDesc, 8) || self.content != null) {
            z10 = true;
        }
        if (z10) {
            output.h(serialDesc, 8, new xj.f(BlockedGroupContent$$serializer.INSTANCE), self.content);
        }
    }
    ...
    public /* synthetic */ BlockedGroupRow(int i10, ChangeType changeType, int i11, int i12, int i13, String str, boolean z10, BlockedGroupButton blockedGroupButton, List list, int i14, DefaultConstructorMarker defaultConstructorMarker) {
        this(i10, changeType, i11, i12, i13, str, z10, blockedGroupButton, (i14 & Spliterator.NONNULL) != 0 ? null : list);
    }
