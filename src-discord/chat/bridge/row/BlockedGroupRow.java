...
public final class BlockedGroupRow extends Row {
    ...
    public static final void write$Self(BlockedGroupRow self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        output.r(serialDesc, 2, self.color);
        output.r(serialDesc, 3, self.borderColor);
        output.r(serialDesc, 4, self.backgroundColor);
        output.t(serialDesc, 5, self.text);
        output.s(serialDesc, 6, self.revealed);
        output.C(serialDesc, 7, BlockedGroupButton$$serializer.INSTANCE, self.button);
        if (output.w(serialDesc, 8) || self.content != null) {
            z10 = true;
        }
        if (z10) {
            output.e(serialDesc, 8, new ti.f(BlockedGroupContent$$serializer.INSTANCE), self.content);
        }
    }
