...
public final class StaticChannelMentionContentNode extends MentionContentNode {
    ...
    public static final void write$Self(StaticChannelMentionContentNode self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        boolean z10 = false;
        output.z(serialDesc, 0, StaticChannelMentionType.Serializer.INSTANCE, self.channelId);
        if (output.A(serialDesc, 1) || self.guildId != null) {
            z10 = true;
        }
        if (z10) {
            output.h(serialDesc, 1, GuildId$$serializer.INSTANCE, self.guildId);
        }
        output.z(serialDesc, 2, new xj.f(ContentNodeSerializer.INSTANCE), self.getContent());
    }
