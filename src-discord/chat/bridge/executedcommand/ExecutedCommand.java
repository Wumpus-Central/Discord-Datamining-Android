...
public final class ExecutedCommand {
    ...
    public static final void write$Self(ExecutedCommand self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        boolean z10 = false;
        output.z(serialDesc, 0, UserId$$serializer.INSTANCE, UserId.m610boximpl(self.userId));
        output.w(serialDesc, 1, self.usernameColor);
        if (output.A(serialDesc, 2) || self.avatarURL != null) {
            z10 = true;
        }
        if (z10) {
            output.h(serialDesc, 2, a2.f32309a, self.avatarURL);
        }
        output.z(serialDesc, 3, AnnotatedStructurableTextSerializer.INSTANCE, self.content);
    }
