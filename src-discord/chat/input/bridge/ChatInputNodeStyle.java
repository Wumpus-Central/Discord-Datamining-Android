...
public final class ChatInputNodeStyle {
    ...
    public static final void write$Self(ChatInputNodeStyle self, CompositeEncoder output, SerialDescriptor serialDesc) {
        ...
        if (z10) {
            output.h(serialDesc, 0, m0.f32384a, self.fontSize);
        }
        output.w(serialDesc, 1, self.color);
        output.h(serialDesc, 2, BackgroundStyle$$serializer.INSTANCE, self.backgroundStyle);
        if (output.A(serialDesc, 3) || self.fontWeight != null) {
            z11 = true;
        }
        if (z11) {
            output.h(serialDesc, 3, ChatInputNodeFontWeight.Companion.serializer(), self.fontWeight);
        }
    }
