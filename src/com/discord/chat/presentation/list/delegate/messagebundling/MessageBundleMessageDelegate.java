...
public final class MessageBundleMessageDelegate extends MessageBundleBaseDelegate {
    ...
    public final MessageView.ChainPart getChainPart(List<?> list, int i) {
        int size = list.size();
        if (size == 0 || size == 1) {
    ...
    public boolean isForViewType(List<? extends MessageBundleItem> items, int i) {
        C9971q.m14633g(items, "items");
        return items.get(i) instanceof MessageBundleMessageItem;
    }
}
